	...
	fire: function (guess) {
		for (var i = 0; i < this.numShips; i++) {
			...
			if (ship.hits[index] === "hit") {
				view.displayMessage("Ой, ты уже попал в это место!");
				return true;
			} else if (index >= 0) {
				ship.hits[index] = "hit";
	...
	processGuess: function (guess) {
		...
		if (location) {
			...
			if (hit && model.shipsSunk === model.numShips) {
				view.displayMessage("Ты потопил все мои боевые корабли за " + this.guesses + " попыток/ки/у");
			}
		}
	}
}
...
function parseGuess(guess) {
	...
	if (guess === null || guess.length !== 2) {
		...
		if (isNaN(row) || isNaN(column)) {
			...
			alert("Ой, это же не по правилам!");
		} else {
			return row + column;
		}
	}
